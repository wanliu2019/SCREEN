...
class CRE {
    version: Version; accession;
    _coord;
    genesAll; genesPC;

    constructor(version: Version, accession: string) {
    ...
    async coord() {
        if (!this._coord) {
            this._coord = DbCommon.crePos(this.version.assembly, this.accession);
        }
    ...
    async topTissues() {
        ...
        const ranks = await DbCommon.creRanks(this.version.assembly, this.accession);
    ...
    async nearbyGenes() {
        ...
        if (!this.genesAll || !this.genesPC) {
            const { genesAll, genesPC } = await DbCommon.creGenes(this.version.assembly, this.accession, coord.chrom);
    ...
    async genesInTad() {
        if ('mm10' == this.version.assembly) {
            return [];
        }
        const coord = await this.coord();
        const rows = await DbCommon.genesInTad(this.version.assembly, this.accession, coord.chrom);
        const c = cache(this.version);
    ...
    async cresInTad() {
        ...
        const coord = await this.coord();
        return DbCommon.cresInTad(this.version.assembly, this.accession, coord.chrom, coord.start);
    }

    async intersectingSnps(halfWindow) {
        return DbCommon.intersectingSnps(this.version.assembly, this.accession, await this.coord(), halfWindow);
    }

    async distToNearbyCREs(halfWindow) {
        return DbCommon.distToNearbyCREs(this.version.assembly, this.accession, await this.coord(), halfWindow);
    }
    ...
    async peakIntersectCount(eset) {
        ...
        return DbCommon.peakIntersectCount(this.version.assembly, this.accession, c.tfHistCounts[eset], eset);
    }
}
...
export async function resolve_credetails(source, args, context, info) {
    ...
    return accessions.map(async (accession) => {
        ...
        const cre = new CRE({versiontag, assembly}, accession);
...
export async function resolve_cre_info(source, args, context, info) {
    ...
    const coord = await cre.coord();
    const c = cache(cre.version);
    const res = await DbCreTable.getCreTable(cre.version.assembly, c.ctmap, {accessions: [cre.accession], range: coord}, {});
    const lookup = c.geneIDsToApprovedSymbol;
    if (res['total'] > 0) {
        return mapcre(cre.version, res['cres'][0], lookup);
    }
    return {};
}
...
export async function resolve_cre_fantomCat(source, args, context, info) {
    ...
    const process = async (key) => {
        const results = await DbCreDetails.select_cre_intersections(cre.version.assembly, cre.accession, key);
...
export async function resolve_cre_ortholog(source, args, context, info) {
    const cre: CRE = source.cre;
    return await DbCreDetails.orthologs(cre.version.assembly, cre.accession);
}
...
export async function resolve_cre_rampage(source, args, context, info) {
    ...
    return getByGene(cre.version, nearest);
}

export async function resolve_cre_linkedGenes(source, args, context, info) {
    const cre: CRE = source.cre;
    if ('mm10' === cre.version.assembly) {
        return [];
    }
    return {
        'linkedGenes': await DbCommon.linkedGenes(cre.version.assembly, cre.accession)
    };
}
...
export async function resolve_cre_tf_dcc(source, args, context, info) {
    ...
    return await DbCommon.tfTargetExps(cre.version.assembly, cre.accession, target, eset);
}
...
export async function resolve_cre_histone_dcc(source, args, context, info) {
    ...
    return await DbCommon.histoneTargetExps(cre.version.assembly, cre.accession, target, eset);
}
...
export async function resolve_cre_miniPeaks(source, args, context, info) {
    ...
    const assembly = cre.version.assembly;
    const options = {
        method: 'POST',
        uri: minipeaks_host,
        body: {
